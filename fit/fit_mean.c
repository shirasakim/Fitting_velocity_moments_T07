...
static double Hubble_z, F_omega, Sigma8, F_omega_norm, Omega_z;

static double params_rho_PDF[3], params_rho_200r[3], params_rho_200t[3];
...
const double pmin1 = 0.01;
const double pmax1 = 100.0;

const double pmin2 = 0.1;
const double pmax2 = 10.0;

const double pmin3 = 0.1;
const double pmax3 = 10.0;
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=7){
    printf("usage: %s input_vpdf input_mass_variance redshift Om Pk_at_z0 output\n", argv[0]);
    return 1;
  }
  ...
  params_rho_200r[0] = pow(5.0, 4);
  params_rho_200r[1] = pow(11.5, 1.3);
  params_rho_200r[2] = 0.50;

  params_rho_200t[0] = pow(7.2, 2.5);
  params_rho_200t[1] = pow(12.6, 0.8);
  params_rho_200t[2] = 0.48;
  ...
  for(iMbin=2;iMbin<nMbin;iMbin++){
	...
	set_data(fname1, index_Mbin_tot, ir);	
      ...
      int ndata = 0;
      for(ir=0;ir<Nrbin;ir++){
	if(D.zz[ir]>0 && D.xx[ir]/tab_Rhalo[jMbin] >= 2.0){
	  ndata += 1;
	}
      }
	...
	chisq = search_best_fit(out);

	if(chisq>0){

	  params_rho_PDF[0] = out[0] * pow(9.4, 2.2);
	  params_rho_PDF[1] = out[1] * (-2.2);
	  params_rho_PDF[2] = out[2] * (1.41);
      
	  FILE *fp;
	  char oname[256];
	  sprintf(oname, "%s_Mbin%d_%d.dat", argv[6], iMbin, jMbin);
	  ...
	  for(ir=0;ir<Nrbin;ir++){
	    ...
	    double p3 = std_r_velocity_param(rbin_fit,  m1, m2);
	...
	chisq = -1e10;
      ...
      if(chisq > 0){
	fprintf(flist, "%e %e %e %e %e %e %e\n", M1_fit, M2_fit, b1_fit, b2_fit, params_rho_PDF[0], params_rho_PDF[1], params_rho_PDF[2]);
      }else{
	fprintf(flist, "%e %e %e %e %e %e %e\n", M1_fit, M2_fit, b1_fit, b2_fit, out[0], out[1], out[2]);
      }
...
double var_func(double delta, double p1, double p2){
  ...
  double res = 200.0 * pow(Omega_z/0.3, 0.6) * (Sigma8 / 0.8) * pow((1+delta)/p1, alpha);
...
void set_data(char *fname_base1, int bin_ID, int bin_rID){
  ...
  if(D.npairs[0] > 100){
    ...
    for(j=0;j<Nvbin;j++){
      ...
      double vel = tab_v[j];
      double yobs;
      yobs = pdf_v_fit[j+1*Nvbin] / (tab_v[1]-tab_v[0]);
    ...
    D.yy[bin_rID] = mean; 
    D.zz[bin_rID] = sqrt(var)/sqrt(D.npairs[0]);
...
double func_search_best_fit(const gsl_vector *v, void *params){
  ...
  params_rho_PDF[0] = p[0] * pow(9.4, 2.2);
  params_rho_PDF[1] = p[1] * (-2.2);
  params_rho_PDF[2] = p[2] * (1.41);
  ...
  for(i=0;i<Nrbin;i++){
    ...
    if(d->xx[i]/rvir >= 2.0 && d->zz[i] > 0){
      ...
      double p3 = std_r_velocity_param(rbin_fit,  M1_fit, M2_fit);

      double p_vr[2] = {p1, p3};
      double p_vt[2] = {p1, p2};

      res += pow(d->yy[i]-moment1_vr(p_vr), 2)/pow(d->zz[i], 2);
